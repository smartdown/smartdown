...
export function isExtensionRegistered(name) {
 ...
 */
export function registerExtension(name, resources, configure) {
 ...
 *
 */
function loadResourceList(thisModule) {
    if (thisModule.resources.length <= thisModule.resourceToLoad) {
        ...
        var callThese = thisModule.loadedCallbacks;
        ...
        var resource = thisModule.resources[thisModule.resourceToLoad];
        if (typeof resource === 'string') {
            var url_1 = resource;
            if (url_1.endsWith('.css')) {
                importCssUrl(url_1, function () {
                    thisModule.resourceToLoad += 1;
                    loadResourceList(thisModule);
                }, function (e) {
                    console.log('loadResourceList importCssUrl error', thisModule.name, url_1, e);
                });
            }
            else {
                importScriptUrl(url_1, function () {
                    thisModule.resourceToLoad += 1;
                    loadResourceList(thisModule);
                }, function (e) {
                    console.log('loadResourceList importScriptUrl error', thisModule.name, url_1, e);
                });
            }
        }
 ...
 *
 */
export function loadExternal(url, isModule, loaded) {
    ...
    if (window.smartdownJSModules[url].loaded) {
        ...
        if (url.endsWith('.css')) {
            importCssUrl(url, function () {
                var callThese = window.smartdownJSModules[url].loadedCallbacks;
